...
import com.aide.ui.re.fragment.EditorPagerAdapter;
import com.aide.ui.re.utils.DensityUtils;
import android.text.TextUtils;
import com.aide.ui.re.view.M3EditText;
import com.aide.ui.re.menuaction.Commands;
import com.aide.ui.re.menuaction.MenuImpl;
...
public class MainActivity extends BaseActivity {
	...
	private EditorPagerAdapter pagerAdapter; // ViewPager2 适配器
	...
	private void setupPager() {
		...
		pagerAdapter = new EditorPagerAdapter(this, openedFiles);
	...
	private void setupRecyclerView() {
			...
			protected void convert(BaseAdapter.BaseViewHolder holder, File item, int position) {
				...
				if (position == selectedFilePosition && !item.isDirectory()) {
					...
					holder.itemView.setBackgroundColor(
							MaterialColors.getColor(holder.itemView, com.google.android.material.R.attr.colorSurface));
				}
			}
		};
		...
		fileAdapter.setOnItemLongClickListener((view, position, item) -> {
			...
			initMenu(popup.getMenu(), Commands.fileBrowserMenuMap);
			popup.setOnMenuItemClickListener(menuItem -> {
				return menuRunner(menuItem.getItemId(), Commands.fileBrowserMenuMap, item);
			});
			popup.show();
		});
	...
	private void saveOpenedFiles() {
		List<String> paths = openedFiles.stream().map(File::getAbsolutePath).collect(Collectors.toList());
	...
	private void loadSavedFiles() {
		...
		if (json != null && !json.isEmpty()) {
			List<String> paths = gson.fromJson(json, new TypeToken<List<String>>() {
			}.getType());
	...
	void initMenu(Menu menu, Map<Integer, MenuImpl> map) {
		for (Map.Entry<Integer, MenuImpl> entry : map.entrySet()) {
			Integer id = entry.getKey();
			MenuImpl impl = entry.getValue();
			MenuItem item = menu.findItem(id);
			if (item != null) {
				item.setEnabled(impl.isEnabled());
				item.setVisible(impl.isVisible());
				item.setTitle(impl.getMenuTitle());
				item.setIcon(impl.getMenuIcon());
			}
		}

	}

	boolean menuRunner(int menuId, Map<Integer, MenuImpl> map, Object... p) {

		MenuImpl impl = map.get(menuId);
		if (impl == null)
			return false;

		if (impl.isVisible() && impl.isEnabled())
			impl.run(this, p);
		return true;
	}
	...
	private void setupMoreMenu() {
		binding.fileBrowserMore.setOnClickListener(v -> {
			...
			popup.setOnMenuItemClickListener(item -> {
				return menuRunner(item.getItemId(), Commands.fileBrowserMenuMap);
			});
			popup.show();
		});
	}
	...
	public void loadFiles(File dir) {
		mmkv.putString("openedDir", dir.getAbsolutePath());
		loadFiles(dir, false); // 默认不强制刷新
	}

	public void loadFiles(File dir, boolean force) {
	...
	private void showTabMenu(int position, View anchor) {
		...
		popup.setOnMenuItemClickListener(item -> {
			...
			if (id == 1)
				...
				closeLeftTabs(position);
			else if (id == 5)
				closeRightTabs(position);
			return true;
		});
		popup.show();
	}
	...
	private void closeRightTabs(int position) {
		if (position >= openedFiles.size() - 1)
			return;
